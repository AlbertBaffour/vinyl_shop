...
class ShopController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        where(function ($query) use ($genre_id) {
            $query->where('id', 'like', $genre_id);
        })
            ->get()[0]['name'];
        $result = compact('genres', 'records','searchedGenre');
    ...
    //Alternative listing page http://localhost:3000/shop_alt
    public function shop_alt()
    {
        $records = Record::with('genre')->orderBy('artist')
            ->get();

        $genres = Genre::orderBy('name')->has('records')        // only genres that have one or more records
